...
def train_model(models, train_data, valid_data, assim_ds, summary_writers,
                log_path, batch_size, epochs, _log, _run,
                _rnd):
    ...
    for epoch in range(epochs):
        ...
        for nr_sample, train_sample in enumerate(train_generator):
            ...
            prior_ens_0 = train_sample['prior_ens_0'][rand_ind].float().to(
                device
            )
            ...
            def autoencoder_closure():
                loss = autoencoder.set_grad(observation=obs, prior=prior_ens_0)
                return loss
            losses_gen = autoencoder.train(
                observation=obs, prior=prior_ens_0, closure=autoencoder_closure
            )
