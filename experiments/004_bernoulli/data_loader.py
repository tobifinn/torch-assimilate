...
from pytassim.model.lorenz_96.obs_ops import BernoulliOperator, obs_ingredient
...
def load_data(base_data_path, normalize, _run, _rnd, rnd_pdf, rnd_kwargs):
    ...
    obs_operator = BernoulliOperator(
        shift=_run.config['obs']['shift'],
        obs_points=_run.config['obs']['obs_points'],
        random_state=_rnd
    )
    ...
    assim_dataset = Lorenz96AssimDataset(
        ...
        rnd_pdf=rnd_pdf, rnd_kwargs=rnd_kwargs, obs_var=0.01
    )
